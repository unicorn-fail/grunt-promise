...
(function (exports, native) {
  ...
  exports.getPromise = function (name) {
    var module, Promise = null;

    // Immediately fail if "name" is not a string.
    if (typeof name !== 'string') this.fail(name);
    ...
    try {
      ...
      [module.Promise, module.defer, module.Deferred, module].forEach(function (obj) {
        if (Promise || !obj) return;
        if (this.isThenable(obj, true)) Promise = obj;
      }.bind(this));
  ...
  exports.load = function (library) {
    ...
    if (library) {
      ...
      if (library !== 'native') this.promises.native = false;
      ...
      if (typeof library === 'function') {
        ...
        else if (!this.isThenable(Promise, true)) this.fail(Promise);
      }

      // Add the library to the modules array.
      if (!Promise) modules.push(library);
    }
    ...
    modules.forEach(function (name) {
      ...
      if (Promise) return;
      Promise = this.getPromise(name);
    }.bind(this));

    if (!Promise) {
      if (library) this.fail(library);
      else {
        if (!this.promises.native) this.fail();
  ...
  exports.suggest = function (limit) {
    ...
    if (limit) suggestions = suggestions.slice(0, limit);
